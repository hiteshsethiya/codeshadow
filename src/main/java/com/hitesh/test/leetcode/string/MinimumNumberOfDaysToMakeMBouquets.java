...
public class MinimumNumberOfDaysToMakeMBouquets {
    ...
    public static int minDays(int[] bloomDay, int m, int k) {
        int currentBouquets = 0, adj = 0, n = bloomDay.length;
        if (m * k > n) return -1;
        int minDays = Integer.MAX_VALUE, left = 0, right = (int) Math.pow(10, 9), pMid = -1;
        while (left <= right) {
            currentBouquets = 0;
            adj = 0;
            int mid = (left + right) / 2;
            if(pMid == mid) break;
            for (int iDay : bloomDay) {
                if (iDay <= mid) {
                    adj++;
                    if (adj == k) {
                        currentBouquets++;
    ...
    public static void execute(int[] bloomDay, int m, int k, int ans) {
        int o = minDays(bloomDay, m, k);
        System.out.println(o);
        System.out.println(o == ans);
    }

    public static void main(String[] args) {
        int[] bloom = new int[]{1,10,3,10,2};
        int m = 3, k = 1;
        execute(bloom, m, k, 3);
        bloom = new int[]{1000000000,1000000000};
        m = 1; k = 1;
        execute(bloom, m, k, 1000000000);
        bloom = new int[]{7,7,7,7,12,7,7};
        m = 2; k = 3;
        execute(bloom, m, k, 12);
        bloom = new int[]{1,10,3,10,2};
        m = 3; k = 2;
        execute(bloom, m, k, -1);
        bloom = new int[]{1,10,3,10,2,1};
        m = 3; k = 2;
        execute(bloom, m, k, 10);
        bloom = new int[]{10,10,30,10,2,1,3,4,5};
        m = 3; k = 1;
        execute(bloom, m, k, 3);
    }
